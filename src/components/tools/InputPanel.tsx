...
export default function InputPanel({ label, value, onChange, placeholder = '', language = 'markdown', useMonaco = false }: InputPanelProps) {
  ...
  return (
    <div className="flex flex-col rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 overflow-hidden" style={{ minHeight: useMonaco ? 420 : undefined }}>
      ...
      >
        {useMonaco ? (
          ...
          <textarea
            ...
            className="w-full min-h-[200px] h-full p-4 font-mono text-sm bg-transparent text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 resize-y focus:outline-none"
            spellCheck={false}
          />
        )}
      </div>
